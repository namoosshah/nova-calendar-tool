    ...
    >
      <slot :uppercaseMode="uppercaseMode" :mode="mode">
        <div class="p-8">
          ...
          <div class="border-b border-40 py-4">
            <label for="doctor" class="mb-2 text-80">Doctor:</label>
            <select v-model="doctor_id" name="doctor" class="w-full form-control form-input form-input-bordered">
              <option value="">Select Doctor</option>
              <option :key="index" :value="doctor.id" v-for="(doctor, index) in doctors">
                {{ doctor.first_name + ' ' + doctor.last_name }}
              </option>
            </select>
          </div>
          <div class="border-b border-40 py-4">
            <label for="patient" class="mb-2 text-80">Patient:</label>
            <select @change="getPatientSessions" v-model="patient_id" name="patient" class="w-full form-control form-input form-input-bordered">
              <option value="">Select Patient</option>
              <option :key="index" :value="patient.id" v-for="(patient, index) in patients">
                {{ patient.first_name + ' ' + patient.last_name }}
              </option>
            </select>
          </div>
          <div class="border-b border-40 py-4">
            <label for="session" class="mb-2 text-80">Live Session:</label>
            <select v-model="live_session_id" name="session" class="w-full form-control form-input form-input-bordered">
              <option value="">Select Live Session</option>
              <option :key="index" :value="session.id" v-for="(session, index) in liveSessions">{{ session.id }}</option>
            </select>
          </div>
        </div>
      </slot>
...
export default {
  name: 'EventModal',
  props: ['currentEvent', 'currentDate', 'patients', 'doctors', 'liveSessions'],
  data() {
    return {
      ...
      doctor_id: this.currentEvent !== null ? this.currentEvent.event.extendedProps.doctor_id.toString() : '',
      patient_id: this.currentEvent !== null ? this.currentEvent.event.extendedProps.patient_id.toString() : '',
      live_session_id: this.currentEvent !== null ? this.currentEvent.event.extendedProps.live_session_id.toString() : ''
    }
  },
  methods: {
    changeStart(value) {
